...
export default function AcademyPage({data}) {
  ...
  return <Layout title={data.title} description={data.description}>
    <div className={data.toc.length ? 'program-layout' : undefined}>
      {data.toc.length > 0 && <aside className="program-sidebar">{data.className?.includes('oa-about') ? <><Link className="program-back" to="/">← Home</Link><p className="sidebar-title">{data.title}</p><nav aria-label="Page sections">{sectionLinks}</nav><Link className="sidebar-book" to="/books">Explore the E-Books ↗</Link></> : <><Link className="program-back" to="/programs">← Learning Programs</Link><p className="sidebar-title">{data.title}</p><nav aria-label="Page sections">{sectionLinks}</nav>{data.key === 'fundamentals' ? <Link className="sidebar-book" to={`/books/${data.key}`}>Read the E-Book ↗</Link> : <span className="sidebar-book sidebar-book--disabled">Read the E-Book (Coming Soon)</span>}<span className="sidebar-status">Udemy course coming soon</span></>}</aside>}
