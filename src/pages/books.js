...
export default function Books(){return <Layout title="E-Books" description="Read Osman Academy's developing collection of AI e-books."><main className="library-page"><header className="library-header"><span className="eyebrow">The Osman Academy library</span><h1>Make room for<br/><em>understanding.</em></h1><p>AI concepts, explained with care. Read at your own pace, work through the examples, and return whenever you need a clearer picture.</p><div className="draft-note"><span className="status-dot"/> A growing collection · All e-books are in draft</div></header><section className="book-grid" aria-label="E-book collection">{books.map(book=><article className={`book-card book-${book.color}`} key={book.id}><Link className="book-cover" to={`/books/${book.id}`} aria-label={`Read ${book.title}`}><div className="cover-brand"><BookLogo/>Osman Academy</div><span className="cover-number">{book.number}</span><div className="cover-title">{book.title}</div><div className="cover-subtitle">{book.subtitle}</div><div className="cover-bottom"><span>E-BOOK / DRAFT EDITION</span><span>↗</span></div></Link><div className="book-info"><h2>{book.title}</h2><p>{book.text}</p><Link className="book-read" to={`/books/${book.id}`}>Read the E-Book <span>→</span></Link><Link className="book-program" to={`/programs/ai-${book.id}`}>Explore the learning program</Link></div></article>)}</section><section className="library-note"><h2>Read. Practice. Explain it back.</h2><p>Our e-books pair detailed explanations with examples and exercises. Corresponding Udemy courses are planned to guide you through the material.</p><Link to="/story#how-we-teach">Discover how we teach →</Link></section></main></Layout>;}
